...
// MIDI_PitchBend maps a pitch bend value onto a MIDI note offset.
func MIDI_PitchBend(val uint16) float32 {
	// 0..8192..16383 maps to -/+ 2 semitones
	return float32(val-8192) * (2.0 / 8192.0)
}

// MIDI_ToFrequency converts a MIDI note to a frequency value (Hz).
// Note: we treat the note as a float for pitch bending, tuning, etc.
